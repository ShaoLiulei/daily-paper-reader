...
## 摘要
我们提出了领域条件元对比学习（Domain-Conditioned Meta-Contrastive Learning），这是一个旨在提高视觉-语言模型跨域泛化能力的框架。虽然像 CLIP 这样的对比模型通过大规模训练取得了强大的性能，但它们依赖于全局目标函数，未能显式地考虑领域偏移（domain shift）。为了解决这一局限性，我们将多模态学习建模为基于领域条件任务的双层元学习（bilevel meta-learning）问题。具体而言，我们引入了用于调节图像和文本表示的领域嵌入（domain embeddings），并通过基于梯度的内循环更新来优化模型，使其能够快速适应特定领域的分布。此外，我们还加入了一种跨域对齐正则化，以促进领域不变表示的生成。我们的方法与标准的对比训练流程兼容，并可应用于涵盖自然和医学领域的异构数据集。我们预期该方法能提高在领域偏移下的鲁棒性，并增强少样本自适应性能，为可扩展的多模态学习指明了一个极具前景的方向。
