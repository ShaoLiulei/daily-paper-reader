...
tags: ["query:ai-llm"]
score: 7.0
evidence: 神经网络权重矩阵的光谱特性与标签噪声诊断
tldr: 本研究探讨了神经网络权重矩阵的谱特性与测试准确率之间的关系。研究发现，瓶颈层特征值分布的尾部指数 alpha 在预测由标签噪声引起的数据质量下降方面具有极高的准确性（R^2=0.984），远超传统指标。尽管它在超参数变化下的泛化预测能力有限，但作为数据质量诊断工具表现出色，能有效识别 CIFAR-10N 中的真实人类标注错误，并与随机矩阵理论中的 BBP 相变建立了联系。
source: arxiv
...
result: 尾部指数 alpha 在检测标签噪声方面表现卓越，但在超参数变化场景下预测能力较弱，且发现层间距比指标在权重矩阵中无效。
conclusion: 尾部指数 alpha 并非通用的泛化预测器，而是一个强大的数据质量诊断工具，能够灵敏地探测训练集的标签污染和退化。
---

## 摘要
我们研究了神经网络权重矩阵的谱性质是否能预测测试准确率。在受控的标签噪声变化下，网络瓶颈层特征值分布的尾部指数 alpha 预测测试准确率的留一法（LOO）R^2 达到 0.984（涵盖 21 个噪声水平，每个水平 3 个随机种子），远超所有基准指标：表现最好的传统指标（最优层的 Frobenius 范数）的 LOO R^2 仅为 0.149。这一关系在三种架构（MLP、CNN、ResNet-18）和两个数据集（MNIST、CIFAR-10）中均成立。然而，在固定数据质量下的超参数变化（180 种配置，涉及宽度、深度、学习率和权重衰减的变化）中，所有谱指标和传统指标的预测能力都很弱（R^2 < 0.25），简单的基准指标（全局 L_2 范数，LOO R^2 = 0.219）略优于谱指标（尾部指数 alpha，LOO R^2 = 0.167）。因此，我们将尾部指数定位为一种数据质量诊断工具：它是标签损坏和训练集退化的强大检测器，而非通用的泛化预测指标。一个在合成噪声上校准的噪声检测器成功识别了 CIFAR-10N 中的真实人类标注错误（检测到 9% 的噪声，误差为 3%）。我们确定信息处理瓶颈层是这一特征的发生地，并将这些观察结果与尖峰随机矩阵模型中的 BBP 相变联系起来。我们还报告了一个负面结果：由于 Wishart 普适性，能级间距比 <r> 对于权重矩阵不具有信息量。
