...
tldr: 本研究将前馈ReLU神经网络建模为细胞层（cellular sheaf），将计算步骤编码为限制映射。通过证明受限拉普拉斯算子的正定性，揭示了前向传播输出本质上是边界数据的唯一调和延拓。该框架支持双向信息传播，允许通过热方程迭代收敛至结果，并实现无需反向传播的局部差异最小化训练。实验验证了该理论在收敛性、约束处理及诊断分析方面的有效性，为神经网络提供了全新的拓扑数学解释。
selection_source: carryover_cache
motivation: 旨在利用代数拓扑中的层论重新解释神经网络，解决传统前向传播缺乏双向信息流和局部诊断能力的问题。
method: 将ReLU网络构建为细胞层，通过受限拉普拉斯算子将计算转化为调和延拓问题，并利用层热方程实现双向信息传播。
result: 证明了前向传播输出的唯一性与热方程的指数级收敛性，并展示了无需反向传播的局部训练及按边诊断的可行性。
conclusion: 该框架为理解神经网络提供了从局部到全局的数学视角，为开发新型训练算法和网络分析工具奠定了理论基础。
---

## 摘要
我们通过为前向传播中的每个中间量设置一个顶点，并将每个计算步骤（仿射变换、激活、输出）编码为边上的限制映射，从任何前馈 ReLU 神经网络构造出一个胞腔层（cellular sheaf）。自由坐标上的受限上边际算子是单位三角矩阵，因此其行列式为 1，且对于每种激活模式，受限拉普拉斯算子都是正定的。由此推导出相对上同调消失，且前向传播输出是边界数据的唯一调和延拓。尽管分段线性激活引入了依赖于状态的切换，层热方程（sheaf heat equation）仍以指数级速度收敛至该输出。与前向传播不同，热方程在层间双向传播信息，从而实现了在两个方向施加约束的固定神经元、无需反向传播的局部差异最小化训练，以及按层和操作类型分解网络行为的逐边诊断。我们在小型合成任务上通过实验验证了该框架，证实了收敛定理，并证明了基于层的训练虽然目前尚无法与随机梯度下降竞争，但遵循理论预测的定量缩放法则。
