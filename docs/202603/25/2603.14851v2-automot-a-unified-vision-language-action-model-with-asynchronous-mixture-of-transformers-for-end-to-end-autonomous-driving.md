...
title_zh: AutoMoT：一种基于异步 Transformer 混合架构的端到端自动驾驶统一视觉-语言-动作模型
...
## 摘要
将视觉-语言模型 (VLM) 集成到端到端 (E2E) 自动驾驶 (AD) 系统中，在提升场景理解能力方面展现出巨大潜力。然而，现有的集成策略存在若干局限性：它们要么难以解决推理空间与动作空间之间的分布失配问题，要么未能充分利用预训练 VLM 的通用推理能力，或者在生成动作策略时产生显著的推理延迟，从而降低了驾驶性能。为应对这些挑战，本文提出了 AutoMoT，这是一个将推理和动作生成统一在单个视觉-语言-动作 (VLA) 模型中的端到端自动驾驶框架。我们的方法利用了具有联合注意力共享的 Transformer 混合 (MoT) 架构，在保留预训练 VLM 通用推理能力的同时，通过在不同任务频率下的异步执行，实现了高效的快慢推理。在开环和闭环设置下，对多个基准测试进行的广泛实验表明，与现有最先进方法相比，AutoMoT 取得了具有竞争力的性能。我们进一步研究了预训练 VLM 在自动驾驶中的功能边界，探讨了何时需要针对自动驾驶进行专门的微调。结果表明，预训练 VLM 仅通过语义提示即可实现具有竞争力的多任务场景理解性能，而对于决策和轨迹规划等动作级任务，微调仍然必不可少。演示视频和定性结果请参阅项目主页。
