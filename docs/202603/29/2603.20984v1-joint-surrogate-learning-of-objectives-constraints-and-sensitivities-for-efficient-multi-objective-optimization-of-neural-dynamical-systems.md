...
## 摘要
生物物理神经系统模拟是计算需求最高的科学应用之一，其优化过程需要在众多约束条件下探索高维参数空间，这些约束施加了二元的“可行/不可行”划分，且缺乏引导搜索的梯度信号。在此，我们介绍了 DMOSOPT，这是一个可扩展的优化框架，它利用统一的联合学习代理模型来捕捉目标、约束和参数灵敏度之间的相互作用。通过学习目标景观和可行性边界的光滑近似，联合代理模型提供了一个统一梯度，能够同时引导搜索朝着更优的目标值和更高的约束满足度方向进行；同时，其偏导数提供了每个参数的灵敏度估计，从而实现了更具针对性的探索。我们在从单细胞动力学到群体水平网络活动的神经回路建模工作流的各个递进阶段验证了该框架，并证明了在超算规模下，该框架能以显著更少的评估次数实现对高度受限问题的高效、有效优化。尽管该框架的动机和演示源于计算神经科学领域，但它是通用的，适用于科学和工程领域的受限多目标优化问题。
