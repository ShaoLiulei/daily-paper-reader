...
## 摘要
目标条件强化学习 (GCRL) 是一种用于学习能够达到任意给定目标策略的框架。特别是，对比强化学习 (CRL) 提供了一个利用对比学习估计的价值函数近似值进行策略更新的框架，与传统方法相比实现了更高的样本效率。然而，由于 CRL 在学习过程中将访问过的状态视为伪目标，它只能针对有限的目标准确估计价值函数。为了解决这个问题，我们为 CRL 提出了一种名为 ViSA（访问状态增强）的新型数据增强方法。ViSA 由两个组件组成：1) 生成增强状态样本，旨在增强同策略探索过程中难以访问的状态样本；2) 学习一致的嵌入空间，通过基于互信息重新表述嵌入空间的目标函数，利用增强状态作为辅助信息来正则化嵌入空间。我们在仿真和真实世界的机器人任务中评估了 ViSA，并展示了改进的目标空间泛化能力，这使得能够对难以访问的目标进行准确的价值估计。更多细节可以在项目页面找到：https://issa-n.github.io/projectPage_ViSA/
