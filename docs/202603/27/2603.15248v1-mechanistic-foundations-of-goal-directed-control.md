...
tldr: 本研究将机械可解释性框架从序列预测扩展到具身控制系统，以婴儿运动学习为模型，揭示了目标导向控制的机制基础。通过分析传感器-认知发育，研究发现归纳偏置催生了因果控制电路，并识别出控制策略转换的相位转换。研究确定了上下文窗口大小是电路形成的关键参数，并提出了描述门控行为的闭式数学模型，为设计可解释的具身智能体提供了理论指导。
selection_source: carryover_cache
motivation: 机械可解释性在序列预测领域已取得成功，但在具身控制系统中仍缺乏类似的机制性解释。
method: 以婴儿运动学习为模型，将机械可解释性框架扩展到传感器-认知发育，研究学习到的门控机制与不确定性阈值。
result: 发现控制电路形成存在相位转换，且上下文窗口大小 k 是关键参数（k≤4 无法形成，k≥8 信心呈对数增长）。
conclusion: 该研究提供了反应式与前瞻式控制策略竞争与演化的机制性说明，为认知发育和可解释具身智能体设计提供了原则性指导。
---
