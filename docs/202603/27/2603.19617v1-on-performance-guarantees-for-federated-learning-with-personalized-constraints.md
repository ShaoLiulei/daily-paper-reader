...
score: 6.0
evidence: 联邦学习算法框架
tldr: 本研究针对联邦学习中客户端存在异构资源或模型约束的实际场景，提出了处理个性化约束的联邦优化框架。本文提出的 PC-FedAvg 算法允许每个客户端拥有私有的凸约束集，通过多块局部决策向量维护其他客户端变量的交叉估计，且仅在自身负责的块上惩罚不可行性，实现了无需共享约束信息或达成全局共识的个性化学习。理论分析给出了次优性和不可行性的通信复杂度界限，实验验证了其在 MNIST 和 CIFAR-10 数据集上的有效性。
selection_source: fresh_fetch
motivation: 针对联邦学习中各客户端存在异构资源或模型约束（私有约束集）而传统方法难以处理的问题。
method: 提出 PC-FedAvg 算法，通过多块局部决策向量和交叉估计机制，让客户端仅对自身约束块进行惩罚，无需共享约束信息。
result: 理论证明了该算法在次优性上达到 O(ε⁻²)、在个体不可行性上达到 O(ε⁻¹) 的通信复杂度。
conclusion: 该方法在不牺牲隐私和共识的前提下，有效解决了带个性化约束的联邦优化问题，并在图像分类任务上得到了验证。
---
