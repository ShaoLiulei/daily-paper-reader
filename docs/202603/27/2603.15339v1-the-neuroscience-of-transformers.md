...
## 摘要
长期以来，神经科学一直为人工神经网络的发展提供启发，但现代架构的成功反过来引发了一个逆向思考：现代网络能否教给我们关于大脑功能的经验？在本文中，我们研究了皮层柱（cortical column）的结构，并提出 Transformer 为皮层微电路组织的多个要素提供了一个自然的计算类比。我们并非声称皮层中存在 Transformer 方程的字面实现，而是建立了 Transformer 操作与皮层层级特征之间的假设映射，并将该类比作为分析和讨论的导向性框架。这种映射使我们能够更深入地研究上下文选择、内容路由、循环整合以及层间变换是如何分布在皮层电路中的。借此，我们针对层级特化、上下文调制、树突整合、振荡协调以及皮层柱的有效连接性，提出了一系列广泛的预测和实验可验证的假设。这一提议旨在作为一个结构化的假设，而非对皮层计算的定论。将 Transformer 操作和皮层构筑学置于一个共同的描述框架中，可以使问题更加明确，揭示新的功能对应关系，并为系统神经科学与现代人工智能之间的互惠交流开辟一条富有成效的路径。更广泛地说，这一视角表明，在计算组织层面比较大脑和架构可以对两者产生真正的见解。
