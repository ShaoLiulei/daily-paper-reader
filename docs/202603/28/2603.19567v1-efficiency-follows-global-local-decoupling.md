...
## 摘要
现代视觉模型必须在不牺牲局部细节的情况下捕捉图像级上下文，同时保持计算成本的可负担性。我们重新审视了这一权衡，并提出了一个简单的原则：将全局推理与局部表示的作用解耦。为了实现这一原则，我们引入了 ConvNeur，这是一种双分支架构，其中轻量级神经记忆分支在紧凑的 token 集上聚合全局上下文，而保持局部性的分支则提取精细结构。一个学习到的门控机制允许全局线索调节局部特征，而不会使它们的目标纠缠在一起。这种分离实现了随图像尺寸呈次二次方增长的缩放特性，保留了与局部处理相关的归纳偏置，并降低了相对于全全局注意力的开销。在标准的分类、检测和分割基准测试中，ConvNeur 在相似或更低的计算量下达到或超过了同类替代方案，并在相似的预算下提供了优异的准确率与延迟权衡。这些结果支持了“效率源于全局-局部解耦”的观点。
