...
title_zh: 具有时变约束的在线凸优化的结构依赖遗憾与约束违反界限
...
score: 6.0
evidence: 在线凸优化的基础理论
tldr: 本研究针对具有时变约束的在线凸优化（OCO）问题，指出传统方法将约束变化视为对抗性过程导致界限过于保守。论文引入了平滑漂移、周期性循环和稀疏切换等结构化特征来描述约束变化，并提出结构自适应原对偶（SA-PD）算法。该算法能在线检测环境结构并调整对偶更新策略，在保持效用的同时显著降低了累积约束违反量，为利用时间规律性优化网络决策提供了理论与实践指导。
selection_source: fresh_fetch
motivation: 现有的在线凸优化理论通常将时变约束视为完全对抗性的，导致在处理具有规律性的实际网络动态时产生的界限过于保守。
method: 提出了一种结构自适应原对偶（SA-PD）算法，通过识别约束信号中的平滑漂移、周期性或稀疏切换等结构特征来动态调整更新策略。
result: "实验表明，SA-PD 算法在电力调度等实际场景中相比于不考虑结构的基准方法，可将累积约束违反量降低高达 53%。"
conclusion: 通过利用约束过程中的时间规律性，可以显著提升受限在线学习算法在复杂网络工程环境中的鲁棒性和性能。
---

## 摘要
具有时变约束的在线凸优化（OCO）是动态网络系统中序列决策的关键框架，学习者必须在满足随轮次变化的可行域的同时最小化累积损失。现有的理论分析通常将约束变化视为单一的对抗过程，导致联合遗憾和违反界限对于现实世界的网络动态过于保守。在本文中，我们引入了约束变化的结构化表征——平滑漂移、周期性循环和稀疏切换——并将这些类别映射到常见的网络现象，如慢信道衰落、昼夜流量模式和离散维护窗口。我们推导了结构依赖的联合界限，当约束过程表现出规律性时，这些界限严格优于对抗速率。为了实现这些收益，我们提出了结构自适应原对偶（SA-PD）算法，该算法利用可观测的约束信号在线检测环境结构，并相应地调整对偶更新策略。在合成基准和真实数据集（包括在线电力调度和变压器负荷管理）上的广泛实验表明，与结构无关的基准相比，SA-PD 将累积约束违反减少了高达 53%，同时保持了具有竞争力的效用。这项工作为在受限在线学习中利用时间规律性进行鲁棒网络工程提供了全面指南。
